...
export function DbMonogram({ className, title = "DekaByte" }: DbMonogramProps) {
  return (
    <svg
      viewBox="0 0 32 32"
      role="img"
    ...
    >
      <rect width="32" height="32" rx="8" fill="currentColor" className="text-graphite" />
      <path
...
export function Logo({ className, markClassName, showWordmark = true }: LogoProps) {
  return (
    <Link
      href="/"
      className={cn(
        "inline-flex items-center gap-2.5 text-graphite transition-opacity hover:opacity-80",
        className,
      )}
    >
      <DbMonogram className={markClassName} />
      {showWordmark ? (
        <span className="text-[1.05rem] font-semibold tracking-tight">DekaByte</span>
      ) : null}
    </Link>
  );
}
